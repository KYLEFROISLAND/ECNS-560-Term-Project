...
In this plot, each point on this plot represents a state for a given year, and the color represents the population bin that the state falls into. We can see that the number of blue (high population) and green (medium population) observations decreases over time, as the number of orange (low population) observations increases over time. This shows that states are generally trending down from higher population bins to lower population bins and this general trend is captured by the downward sloping local regression line.
