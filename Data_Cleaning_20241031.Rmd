...
Research Question 3: Can we predict population decline using Location, Hunting Data, and Public Land Percentage? 
```{r}
library(tidymodels)
library(ISLR)
library(glmnet)
...
set.seed(123)
bison_split = distance_hunting_land2 |> initial_split(prop = 0.7)
bison_train = distance_hunting_land2 |> training()
bison_test = distance_hunting_land2 |> testing()

#Define a Recipe:
bison_recipe = recipe(difference ~ distance + total_population + 'Paid Hunting License Holders' + public, data = bison_train) |> 
  update_role(GEOID, new_role = "id") |> 
  step_poly(all_numeric_predictors(), degree = 2) |> #create second term for each numeric var that is a quadratic
  step_interact(~all_numeric_predictors():all_nominal_predictors()) |> #create interactions between numeric and indicator variables
  step_dummy(all_nominal_predictors()) |> #create indicators for each categorical or character variable
  step_nzv(all_predictors())#remove predictors with nearly 0 variance... otherwise not very reliable because difficult to pickup patterns

#Process the Data:
bison_clean = bison_recipe |> prep() |> juice() 

#Define linear model:
model_lm = linear_reg() |> set_engine("lm") 

#Define cross validation for experimentation with other regression forms later
bison_cv = bison_train |> vfold_cv(v=5)

#Define Workflow:
fit_lm_cv = workflow() |>
  add_model(model_lm) |>
  add_recipe(bisont_recipe) |>
  fit_resamples(bison_cv)
...
ggplot(distance, aes(x = log(distance), y = difference)) + 
  ...
  labs(title = "Latitude and Bison Population Decline") +
  geom_point()
#There does not seem to be as clear of a relationship from this plot as I had originally expected. The predictive power of this variable seems tenuous.
```

Research Question 2: Can we predict population decline using Latitude and hunting data? This probably won't work due to limited data points...
```{r}
library(tidyverse)
library(tidymodels)
library(ISLR)
...
#First, split into train and test data
set.seed(123)
bison_split = distance_hunting |> initial_split(prop = 0.7)
bison_train = distance_hunting |> training()
bison_test = distance_hunting |> testing()

#Define a Recipe:
bison_recipe = recipe(difference ~ disatnce + total_population + Paid Hunting License Holders, data = bison_train) |> 
  update_role(GEOID, new_role = "id") |> 
  step_poly(all_numeric_predictors(), degree = 2) |> #create second term for each numeric var that is a quadratic
  step_interact(~all_numeric_predictors():all_nominal_predictors()) |> #create interactions between numeric and indicator variables
  step_dummy(all_nominal_predictors()) |> #create indicators for each categorical or character variable
...
ggplot(data_long %>% filter(state_code == "MONTANA"), aes(x = Year, y = Count, color = License_Type)) +
  ...
  theme_minimal()
```
>>>>>>> ae5d3f30ef6b3c153d8c3a489f3f9dc3d1e461cb
=======
>>>>>>> Stashed changes
